...
const ModalConfirmationDeleteUser = ({ open, onClose, companyName}) => {
  ...
  const handleDelete= async()=>{
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setSeverity('error');
        setSuccessMessage('Deleting User Failed! reloading in 2 second');
        setSnackbarOpen(true);
        await new Promise(resolve => setTimeout(resolve, 1000)); // Wait for 2 seconds
        location.reload();
      ...
      setSeverity('error');
      setSuccessMessage('Deleting User Failed! Refreshing in 2s');
      setSnackbarOpen(true);
      await new Promise(resolve => setTimeout(resolve, 1000)); // Wait for 2 seconds
      location.reload();
      // Handle the error, maybe show an error message to the user
    ...
    >
     <div style={paperStyle}>
        ...
        <Grid container spacing={1} style={columnStyle}>
         <Grid item xs={12}><Typography>Apakah anda yakin akan Menghapus 0{companyName}?</Typography></Grid>
...
export default ModalConfirmationDeleteUser;
